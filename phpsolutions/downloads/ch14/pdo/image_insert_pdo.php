...
if (array_key_exists('insert', $_POST)) {
  ...
  // carry only if input OK
    ...
    $checkUnique = "SELECT COUNT(*) FROM images
                    WHERE filename = ?";
    // initialize prepared statement
    $stmt = $conn->prepare($checkUnique);
    ...
    // if not a duplicate, proceed with insertion
      ...
      // pass parameters as array and execute
      $OK = $stmt->execute(array($filename, $caption));
	  // free the statement for the next query
      $stmt->closeCursor();
      // STEP 5
...
<form id="form1" name="form1" method="post" action="">
  ...
  <p>
	...
	foreach ($conn->query($allCats) as $row) {
	?>
