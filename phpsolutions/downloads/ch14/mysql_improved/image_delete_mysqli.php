...
if (isset($_GET['image_id']) && !$_POST) {
  ...
  if ($image_id) {
    ...
    $result = $conn->query($getDependents) or die (mysqli_error($conn));
	...
	$result = $conn->query($getImage) or die(mysqli_error($conn));
...
if (array_key_exists('delete', $_POST)) {
  ...
  $deleted = $conn->query($sql) or die(mysqli_error($conn));
  }
